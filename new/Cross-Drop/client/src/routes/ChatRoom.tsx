...
function CopyRoomLink({ roomID }: { roomID: string | undefined }) {
  ...
  function handleCopy() {
    ...
    // Build full room link dynamically
    const link = `${window.location.origin}/room/${roomID}/chat`;
...
function ChatRoom() {
  ...
  const [username, setUsername] = useState(() => {
    const saved = localStorage.getItem("peershare-username");
  ...
  function saveUsername() {
    ...
    setUsername(newName);
    localStorage.setItem("peershare-username", newName);
    setEditing(false);
  }
